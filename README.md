...
当前小程序主线是“口味测试 -> 选本推荐 -> 打本工具 -> 个人档案”。搭子匹配、找局和组局入口已下线，本轮不恢复这些功能。
...
AI_API_KEY=your-opencode-zen-key
AI_RETRY_COUNT=1
AI_DAILY_COST_LIMIT=0
```

DeepSeek V4 Flash 真实模型可设置：
...
AI_RETRY_COUNT=1
AI_DAILY_LIMIT=20
AI_DAILY_COST_LIMIT=30
```

真实模型接入时，模型 Key 只能放在后端 `.env` 的 `AI_API_KEY`。配置完成后，可在后端目录执行 `npm run smoke:ai-provider` 做一次真实供应商预检；脚本只输出请求模型、实际路由模型、响应长度和 token/cost 元数据，不打印 key 或生成正文。
...
`npm start` 强制使用生产模式：`JWT_SECRET` 必须为至少 32 字符的非示例密钥，且不能开启微信开发登录。配置不合格时服务拒绝启动；本地联调使用 `npm run dev`。

AI 请求先在 SQLite 事务中预占额度，完成后结算。`AI_USER_CONCURRENCY` 默认 1，`AI_GLOBAL_CONCURRENCY` 默认 4。启用成本预算时，按 `AI_COST_RESERVE_PER_REQUEST` 预占每次调用的预估成本（默认 0.01，单位与供应商 `cost_credits` 相同）；应按实际模型费用调整。预估预占不能替代供应商的硬账单上限，单次实际费用超出预估时仍可能超预算。过期请求按预估成本保守计入，避免服务重启后重复花费。
